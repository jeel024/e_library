...
class UpdateCategory1 extends StatefulWidget {
  const UpdateCategory1({Key? key}) : super(key: key);

  @override
  State<UpdateCategory1> createState() => _UpdateCategory1State();
}
class _UpdateCategory1State extends State<UpdateCategory1> {
  ...
  Widget build(BuildContext context) {
    return GetBuilder<Mycontroll>(builder: (controller) {
      ...
      print(BlocProvider.of<AuthBloc>(context).genderModel!.category);
        controller.ctgy.value = BlocProvider.of<AuthBloc>(context).genderModel!.category!;
        controller.gender.value = BlocProvider.of<AuthBloc>(context).genderModel!.gender!;
                ...
                InkWell(
                    onTap: () {
                      ...
                      if (controller.ctgy.isNotEmpty) {
                        if(BlocProvider.of<AuthBloc>(context).genderModel!.category==null)
                        {
                          BlocProvider.of<AuthBloc>(context).add(StoreGender());
                        }
                        else
                        {
                          BlocProvider.of<AuthBloc>(context).add(UpdateCategory());
                        }
